...
export async function insertComment(comment) {
  db.prepare(
    `
      INSERT INTO COMMENT(post_no, user_id, content, input_datetime) 
      VALUES(?, ?, ?, CURRENT_TIMESTAMP)
    `
  ).run(comment.postNo, comment.userId, comment.content);
}
...
export async function updateComment(comment) {
  db.prepare(
    `
      ...
      AND user_id = ?
    `
  ).run(comment.content, comment.user_id, comment.post_no, comment.user_id);
}
...
export async function deleteComment(comment) {
  db.prepare(
    `
      ...
      AND user_id = ?
    `
  ).run(comment.post_no, comment.user_id);
}
export default db;
