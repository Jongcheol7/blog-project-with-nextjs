    ...
    .prepare(
      ...
      WITH RECURSIVE CATEGORY_TREE AS (
        SELECT 
          ...
          name,
          0 AS depth,
          CAST(category_id AS TEXT) AS path
        FROM CATEGORY 
...
export async function deleteBlogTags(postNo) {
  return db
    .prepare(
      `
      UPDATE BLOG_TAGS SET del_datetime = CURRENT_TIMESTAMP WHERE post_no = ?
    `
    )
    .run(postNo);
}
  ...
  return db
    .prepare(
      ...
      WHERE EXISTS (
        SELECT 1
        FROM BLOG_TAGS
        WHERE BLOG_TAGS.tag_id = TAGS.tag_id
          AND BLOG_TAGS.post_no = ?
      )
    `
    )
    .run(postNo);
...
// 유저 닉네임 조회
export async function getUserNickName(id) {
  return db.prepare(`SELECT nick_name FROM USERS WHERE id=?`).get(id);
}

// 닉네임 중복검사 조회
export async function getDuplicatedNickname(nickname) {
  return db
    .prepare(
      `
      SELECT * FROM NICKNAME WHERE nick_name = ?
    `
    )
    .get(nickname);
}
...
// 닉네임 테이블 insert
export async function insertNickname(nickname, userId) {
  return db
    .prepare(
      `
      INSERT INTO NICKNAME (nick_name, user_id, input_datetime) 
      VALUES(?,?,CURRENT_TIMESTAMP)
    `
    )
    .run(nickname, userId);
...
// USER 테이블 닉네임 update
export async function updateUserNickname(nickname, userId) {
  return db
    .prepare(
      `
